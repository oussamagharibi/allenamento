...
// Analisi di una foto di un pasto: il modello deve restituire SOLO JSON.
function sistemaFoto(piano) {
  const allergeni = piano && (piano.allergeni || []).length ? piano.allergeni.join(', ') : 'nessuno segnalato';
  return SISTEMA_BASE +
    '\n\nCompito: guardare la foto di un pasto e stimare che cosa contiene.\n\n' +
    'FORMATO DELLA RISPOSTA: rispondi SOLO con JSON valido. Nessun testo prima o dopo, ' +
    'nessun backtick, nessun blocco di codice, nessuna spiegazione.\n\n' +
    'Struttura richiesta:\n' +
    '{"descrizione":"frase breve sul piatto","alimenti":[{"nome":"riso","porzione_g":150}],' +
    '"calorie":650,"proteine":30,"carboidrati":80,"grassi":18,"confidenza":"media"}\n\n' +
    'Regole:\n' +
    '- "confidenza" vale "alta" se il piatto e ben riconoscibile, "media" se stimi le porzioni, ' +
    '"bassa" se la foto e poco chiara o il cibo e coperto;\n' +
    '- i valori sono numeri interi, riferiti a tutto quello che si vede nella foto;\n' +
    '- resta nei limiti realistici di un pasto: calorie 0-3000, proteine 0-300 g, carboidrati 0-600 g, grassi 0-300 g;\n' +
    '- se nella foto non si vede cibo, metti confidenza "bassa", tutti i valori a 0 e spiegalo nella descrizione;\n' +
    '- non dare consigli e non commentare le scelte di chi mangia: qui devi solo stimare.\n' +
    '- alimenti da tenere presenti per allergie dichiarate: ' + allergeni + '. Se ti sembra che siano nel piatto, ' +
    'segnalalo nella descrizione senza allarmismi.';
}
...
function datiUtente(contesto) {
  ...
  const dati = {
    ...
    diario_ultimi_7_giorni: contesto.diario || 'nessuna registrazione',
...
module.exports = { SISTEMA_BASE, sistemaAnalisi, sistemaChat, sistemaScheda, sistemaAlimentazione, sistemaFoto, datiUtente };
